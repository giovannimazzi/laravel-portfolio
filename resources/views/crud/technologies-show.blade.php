@extends('layouts.admin')

<x-modal :entity="$technology"></x-modal>

@section('content')

    <div class="card shadow-sm">
        
        <div class="card-header bg-primary text-white d-flex justify-content-between align-items-center">
            <h3 class="mb-0">Technology Details</h3>
            <a href="{{route("technologies.index")}}" class="btn btn-outline-dark">
                <i class="bi bi-chevron-double-left"></i>
                Back to List
            </a>
        </div>
        <div class="card-body">
            <h5>Name:</h5>
            <p class="fw-bold"><big>{{$technology->name}}</big></p>
            <hr/>
            <h5>Color:</h5>
            <span class="badge" style="background-color: {{$technology->color}}">{{$technology->color}}</span>
            <hr/>
            <div class="d-flex justify-content-end gap-2">
                <a href="{{route('technologies.edit', $technology)}}" class="btn btn-outline-success" title="Edit"><i class="bi bi-pencil"></i></a>
                <button type="button" class="btn btn-outline-danger" data-bs-toggle="modal" data-bs-target="#m{{$technology->id}}" title="Delete"><i class="bi bi-trash"></i></button>   
            </div>         
        </div>
    </div>

@endsection